...
1. [🚀 System Architecture](#system-architecture)
    ...
    * [3. Concurrent Operational Flow](#concurrent-flow)
    * [4. State Machine & Precision Integration](#state-machine)
...
| **Fig 2** | Concurrent Sequence Flow (Thread Interaction) | [View Section](#concurrent-flow) |
| **Fig 3** | Aircraft State Transition Logic (State Machine) | [View Section](#state-machine) |
| **Fig 4** | Local Jitter Comparison (Fixed vs. Compensated) | [View Section](#performance-audit) |
| **Fig 5** | CI Execution Report (GitHub Actions Logs) | [View Section](#performance-audit) |
...
* **Orchestration**: The `Simulator` acts as the root controller, managing the lifecycle of `Aircraft` threads and shared hardware resources. This top-down ownership ensures clean thread lifecycle management.
* **Encapsulation**: Each `Aircraft` instance is self-contained. It references `AircraftConfig` for its manufacturer-specific "DNA" (speed, battery capacity, etc.) and updates its own `AircraftStats` telemetry in real-time.
* **Resource Arbitration**: The `ChargerPool` is entirely decoupled from aircraft logic. It abstracts the 3 hardware chargers using **`std::counting_semaphore`**, allowing aircraft to request access without needing to know the internal implementation of the queue.
...
### 3. Concurrent Operational Flow (Thread Interaction)
The engine drives 20 aircraft in parallel, reflecting a true concurrent environment. **Figure 2** illustrates the interaction between an aircraft's execution thread and the shared resource pool:

<p align="center">
  <b>Figure 2: Concurrent Operational Flow (Sequence Diagram)</b><br>
  <img src="image/concurrency-sequence-diagram.png" width="70%">
</p>

* **Independent Ticks**: Each aircraft thread independently triggers its `update()` method. While in the `Flying` state, it consumes battery based on its cruise speed and energy usage profile.
* **The Transition Gap**: When the battery is depleted, the thread does not stall; it immediately enters the **Resource Negotiation** phase by performing a non-blocking `try_acquire()` on the **`std::counting_semaphore`**.
* **Dynamic Response**: 
    * **Acquisition Success**: The aircraft secures a charger and transitions to the `Charging` state immediately.
    * **Acquisition Failure**: The aircraft logs a transition to `Waiting`. Crucially, because the check is non-blocking, the thread remains active and attempts again in the next tick, allowing for the precise accumulation of `wait_time` metrics.

<a id="state-machine"></a>
### 4. State Machine & Precision Integration
To ensure the simulation honors the "three-hour data" requirement, we handle mid-tick state changes with mathematical rigor. **Figure 3** reveals the cyclic nature of the vehicle lifecycle:

<p align="center">
  <b>Figure 3: Aircraft State Transition Logic</b><br>
  <img src="image/aircraft-state-machine.png" width="55%">
</p>

* **Atomic State Shifts**: Vehicles cycle through **Flying → Waiting → Charging → Flying**.
* **Eliminating Time Leakage**: A common pitfall in discrete simulations is "over-shooting" a state change. We implement a **Precision Loop** to handle boundary conditions. For example, if a Charlie model has exactly **0.625h** of flight endurance remaining but the current simulation tick $\Delta t$ is **1.0h**, the engine processes:
    1.  **0.625h of Flight** (depleting the battery to exactly zero).
    2.  The remaining **0.375h** of the tick is **seamlessly handed off** to the `Waiting` or `Charging` logic within the same cycle.
* **Result**: This ensures every microsecond of the 3-hour simulation is accounted for in one of the three states, resulting in a perfect summation of Flight + Wait + Charge times.
...
<table border="0" align="center">
 ...
 <tr>
    <td colspan="2" align="center"><b>Figure 4: Local Execution Comparison (Temporal Drift vs. Precision Sync)</b></td>
 </tr>
</table>
...
<p align="center">
  <b>Figure 5: GitHub Actions CI Execution Report</b><br>
